...
void doDeleteCar(Database& inDB);        
void doModifyMileage(Database& inDB);    
...
int displayMenu()
{
    ...
    cout << "7) Delete a car (Database 1)" << endl;
    cout << "8) Modify mileage of a car (Database 1)" << endl;
    cout << "9) Delete a car (Database 2)" << endl;
    cout << "10) Modify mileage of a car (Database 2)" << endl;
    cout << "0) Quit" << endl;
    cout << endl;
...
void doDeleteCar(Database& inDB)
{
    ...
    cin >> carCode;
    try {
        inDB.deleteCar(carCode);
    } catch (std::exception& ex) {
        cerr << "Unable to delete car!" << endl;
    }
}
...
void doModifyMileage(Database& inDB)
{
    int carCode;
    double newMileage;
    cout << "Enter car code to modify mileage: ";
    cin >> carCode;
    cout << "Enter new mileage: ";
    cin >> newMileage;
    try {
        inDB.modifyMileage(carCode, newMileage);
    } catch (std::exception& ex) {
        cerr << "Unable to modify mileage!" << endl;
    }
}
