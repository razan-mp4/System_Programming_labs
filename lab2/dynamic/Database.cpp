...
void Database::deleteCar(int carCode)
{
    for (int i = 0; i < mNextSlot; i++) {
        if (mCars[i].getCode() == carCode) {
            ...
            for (int j = i; j < mNextSlot - 1; j++) {
                mCars[j] = mCars[j + 1];
            }
            mNextSlot--;
            cout << "Car with code " << carCode << " deleted successfully." << endl;
            return;
        }
    }
...
void Database::modifyMileage(int carCode, double newMileage)
{
    for (int i = 0; i < mNextSlot; i++) {
        if (mCars[i].getCode() == carCode) {
            mCars[i].setMileage(newMileage);
            cout << "Mileage for car with code " << carCode << " modified successfully." << endl;
            return;
        }
    }
